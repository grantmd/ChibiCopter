...
int sound_init(void){
	...
	/* sending */
  i2cAcquireBus(&I2CD1);
	i2cMasterTransmit(&I2CD1, sound_addr, sound_tx_data, 4, sound_rx_data, 0, &errors, TIME_INFINITE);
  i2cReleaseBus(&I2CD1);
...
void sound_read(void){
  ...
  i2cAcquireBus(&I2CD1);
  i2cMasterTransmit(&I2CD1, sound_addr, sound_tx_data, 1, sound_rx_data, 6, &errors, TIME_INFINITE);
  i2cReleaseBus(&I2CD1);
